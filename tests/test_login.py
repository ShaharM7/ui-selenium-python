...
def navigator():
    ...
    sign_in_page = SignInPage(browser=browser, awaiter=awaiter)
    github_home_page = GitHubHomePage(browser=browser, awaiter=awaiter, sign_in_page=sign_in_page)

    page_navigator = PageNavigator(browser=browser, github_home_page=github_home_page, sign_in_page=sign_in_page)
...
class TestLoginInPage:
    def test_github_home_page(self, navigator):
        home_page = navigator.navigate_to_home_page()
        ...
        assert "Incorrect username or password." == sign_in_page.get_error_incorrect_message().strip()
